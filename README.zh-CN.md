...
| 特性 | 说明 |
|------|------|
| **诊断实验** | Phase 2.5：结构化假设-证伪循环，在修改代码前注入破坏性诊断代码、收集硬数据、git checkout 清理 |
| **根因门控** | Phase 2.7：硬门控，根因未证明时阻断 Phase 3（定向修复），需支持证据 + 至少1个证伪实验 |
| **已排除假设** | `excluded_hypotheses.json` 跨 Fix Loop 迭代持久化——新 Engineer 收到已证伪假设，避免重复失败方向 |
| **主动证伪** | 诊断实验必须包含至少1个反证实验，能证伪主导假设——打破确认偏见 |
| **诊断清理** | Phase 2.5 结束后 `git checkout .` 擦除所有注入的诊断代码，确保 Phase 3 从干净状态开始 |

### 设计权衡（v0.4.1 → v0.4.2）

v0.4.2 升级源于一次真实事故：Tab Bar 图标裁切问题被"修复"8+ 次仍未解决。根因分析发现：

1. **8 轮迭代全部猜错原因**——都假设"盒模型重叠"，从未实测 DOM。Playwright boundingBox 后来证明存在 2px 正间隙（根本不重叠）。
2. **确认偏见**——一旦假设"FILL=1 导致渲染异常"，后续实验只寻找支持证据，从不寻找反证。如果测试全部 5 个图标的 FILL=1，就会发现只有 `home` 图标受影响。
3. **记忆断代丢失关键负向知识**——新 Engineer 不知道"盒模型重叠"已被证伪，于是再次尝试。

**关键权衡：**

| 决策 | 选择 | 放弃 | 原因 |
|------|------|------|------|
| 根因验证 | 结构化诊断实验（Phase 2.5） | 信任 Bug Analysis 推理 | 读代码 ≠ 测量现实；8轮"修复"证明纯推理不可靠 |
| 代码修改门控 | 根因门控（Phase 2.7） | 允许 Engineer 立即修复 | 未证明的根因 = 错误修复 = 浪费迭代和代码退化 |
| 证伪要求 | 至少1个反证实验 | 仅要求支持证据 | 确认偏见是头号失败模式；主动证伪打破循环 |
| 跨迭代知识 | excluded_hypotheses.json | 携带完整历史 | 完整历史膨胀上下文；只有负向知识（不要试什么）是必要的 |
| 诊断代码注入 | 显式授权（破坏性） | 禁止 | 隔离实验需要临时代码变更；Git Checkpoint 保证回滚 |
| 诊断清理 | Phase 2.5 后 `git checkout .` | 手动清理 | 手动清理易出错；git checkout 是原子操作，保证干净 |
| 反对者 Agent | 内嵌在诊断实验中 | 独立 Agent | 证伪步骤达到同样目标，无需协调开销 |
